# -*- coding: utf-8 -*-
"""
Created on Sat Sep 28 21:35:20 2019
...
def calc_candidate_total_cost(candidate, param_for_n_generators, pdemand):
    fuel_cost = calc_candidate_fuel_cost(candidate, param_for_n_generators)
    emission_cost = calc_candidate_emission_cost(candidate, param_for_n_generators)
    hi_val = calc_hi(param_for_n_generators, candidate, pdemand)
...
def initiate_population_random(param_for_n_generators, pop_size, gen_no, pdemand):
    pi_params = param_for_n_generators[param_for_n_generators.columns[6:8]]
...
def calculate_population_cost(popln, param_for_n_generators, pdemand):
    ...
    for i in range(pop_size):
        ...
        initial_pop_cost[i][gen_no] = calc_candidate_total_cost(popln[i], param_for_n_generators, pdemand)
...
def whole_linear_crossover(parent_popln_cost, param_for_n_generators, pdemand):
    ...
    child_popln_cost_copy = calculate_population_cost(child_popln_cost_copy[:,:-1],param_for_n_generators, pdemand)
...
def mutation(child_popln_cost, param_for_n_generators, pdemand, prob_mutation):    
    pi_params = param_for_n_generators[param_for_n_generators.columns[6:8]]
    ...
    mutated_popln_cost = calculate_population_cost(mutated_popln_cost[:,:-1],param_for_n_generators, pdemand)
